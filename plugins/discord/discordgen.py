...
class NitroGen: # Initialise the class
    ...
    def main(self): # The main function contains the most important code
        ...
        if os.name == "nt": # If the system is windows
            print("")
            ctypes.windll.kernel32.SetConsoleTitleW("Nitro Generator and Checker - Made by RageWire#0001") # Change the
...
╚═╝  ╚═╝╚═╝  ╚═══╝ ╚═════╝ ╚═╝  ╚═══╝╚═╝╚═╝  ╚═╝
        ...
        self.slowType("Made by: RageWire#0001", .02) # Print who developed the code
        ...
        for i in range(num): # Loop over the amount of codes to check
            ...
            if os.name == "nt": # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(f"Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by RageWire#0001") # Change the title
                print("")
            else: # If it is a unix system
                print(f'\33]0;Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made byRageWire#0001\a', end='', flush=True) # Change the title
...
Results:
 ...
 Valid Codes: {', '.join(valid )}""") # Give a report of the results of the check
        ...
        input("\nThe end! Press Enter 5 times to go back to the meny.") # Tell the user the program finished
